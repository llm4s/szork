        ...
        case Right(audioUrl) =>
          downloadAndEncodeAudio(audioUrl) match {
            case Right(base64Audio) =>
              ...
              logger.info(
                s"Music generation completed in ${musicGenerationTime}ms (${base64Audio.length} bytes base64)")
