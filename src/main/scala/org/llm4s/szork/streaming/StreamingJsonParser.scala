package org.llm4s.szork.streaming

import org.slf4j.LoggerFactory

/** Progressively parses streaming JSON to extract narrationText as it arrives. Handles incomplete JSON by attempting to
  * close quotes and braces.
  */
class StreamingJsonParser {
  ...
  /** Process a new chunk of JSON and attempt to extract any new narration text.
    ...
    *   The new JSON chunk
    * @return
    *   Option containing any newly available narration text
    */
  ...
  /** Extract narrationText from potentially incomplete JSON. Returns only the newly extracted portion since last call.
    */
  ...
  /** Attempt to extract narrationText by adding closing quotes/braces if needed. Called when streaming is complete but
    * we might have incomplete JSON.
    */
