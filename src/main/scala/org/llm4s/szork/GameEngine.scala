...
class GameEngine(
  ...
  def shouldGenerateBackgroundMusic(responseText: String): Boolean =
    CoreEngine.shouldGenerateBackgroundMusic(core, responseText)
  ...
  def restoreGameState(state: GameState): Unit = {
    ...
    if (state.agentMessages.nonEmpty) {
      ...
      logger.info(
        s"[$sessionId] Game state restored (legacy) with ${core.conversationHistory.size} conversation entries")
    } else {
