...
object GameStateCodec {
  ...
  def toJson(state: GameState): Value = {
    Obj(
      ...
      "currentScene" -> state.currentScene.map { scene =>
        Obj(
      ...
      "conversationHistory" -> state.conversationHistory.map(e => Obj("role" -> e.role, "content" -> e.content, "timestamp" -> ts(e.timestamp))),
  ...
  def fromJson(json: Value): GameState = {
    GameState(
      ...
      conversationHistory = json("conversationHistory").arr.map(e => ConversationEntry(e("role").str, e("content").str, readTs(e("timestamp")))).toList,
      ...
      adventureTitle = json.obj.get("adventureTitle").flatMap { case Null => None; case s => Some(s.str) }.filter(_.nonEmpty),
      adventureOutline = json.obj.get("adventureOutline").flatMap { case Null => None; case v => Some(GamePersistence.parseAdventureOutlineFromJson(v)) },
      agentMessages = json.obj.get("agentMessages").map(_.arr.toList).getOrElse(Nil),
      mediaCache = json.obj.get("mediaCache").map { co =>
  ...
  def metadataFromJson(json: Value): GameMetadata = {
    ...
    GameMetadata(
      ...
      adventureTitle = json.obj.get("adventureTitle").flatMap { case Null => None; case s => Some(s.str) }.filter(_.nonEmpty).getOrElse("Untitled Adventure"),
