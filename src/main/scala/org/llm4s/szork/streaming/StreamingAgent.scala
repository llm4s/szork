...
/** Extension of Agent that supports streaming responses. Only streams user-facing text, not tool calls or internal
  * operations.
  */
class StreamingAgent(client: LLMClient) extends Agent(client) {
  ...
  /** Run a single step of the agent's reasoning process with streaming support.
    ...
    * @param onUserText
    *   Callback for streaming user-facing text chunks
    * @return
  ...
  private def handleInProgressStreaming(
    ...
    logger.debug("Running streaming completion step with tools: {}", state.tools.tools.map(_.name).mkString(", "))
  ...
  /** Run the agent until completion with streaming support. Handles the multi-step nature of agent reasoning while only
    * streaming user-facing responses.
    *
    * @param initialState
    *   The initial agent state
    * @param onUserText
    *   Callback for streaming user-facing text
    * @return
